...
from fantasy_nba.nba_utils import fetch_api_data
...
def fetch_all_stats(urls: List[str], dates: list, home_teams: list, away_teams: list) -> DataFrame:
    responses = asyncio.run(fetch_api_data(urls))
...
def parse_html_tables(html: str, date, home_team_ab: str, away_team_ab: str) -> pd.DataFrame:
...
def calc_expanding(stats: pd.DataFrame) -> pd.DataFrame:
    expanding_mean = stats[["Player", "GameDay", "Points"]].groupby(["Player"]).expanding().mean()
...
def calc_rolling(stats: pd.DataFrame, windows: List[int], forward: bool = False) -> pd.DataFrame:
