...
def app():
    ...
    with st.sidebar:
        ...
        a,b = st.columns(2)
        MAX_TRADES = a.number_input(label="Max Trades", format="%i", min_value=1, max_value=7, step=1)
        MAX_SLOTS = b.number_input(label="Max Slots", format="%i", min_value=1, max_value=5, step=1)

    solution = find_optimal_solution(max_slots=MAX_SLOTS, max_trades=MAX_TRADES, week=PERIOD)
    ...
    d.area_chart(data=solution[0]["Total"], height=250)#, index=solution[0]["Day"])
