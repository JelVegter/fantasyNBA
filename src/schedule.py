...
class Schedule:
    ...
    def refresh_data(self):
        games = schedule_builder(self.year, self.months)
        games.to_csv("data/schedule.csv")
        print("Schedule data updated...")
        return games


def retrieve_schedule() -> DataFrame:
    """Function to retrieve locally stored NBA schedule"""
    games = read_csv("data/schedule.csv")
    games["Date"] = to_datetime(
        games["Date"], errors="coerce"
    )  # .dt.tz_localize(tz='US/Eastern') ###TODO CHECK IF TIMEZONES AREN'T FUCKED NOW!
    return games
